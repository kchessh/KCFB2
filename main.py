...
new_teams = []
with open("Teams.txt", encoding='latin-1') as file:
	text = file.read()
	teams = text.split(",")
	for team in teams:
	...
	for team in new_teams:
		...
		while i < len(week_data):
			...
			if winner == team.replace("%26", "&"):
				score += 1
				try:
					with open(f"Team_Results/{team}.txt", 'r', encoding='latin-1') as file:
						text = file.read()
						results = text.split(",")
				except FileNotFoundError:
					results = []
				with open(f"Team_Results/{team}.txt", 'a+', encoding='latin-1') as file:
					if len(results) == 0:
